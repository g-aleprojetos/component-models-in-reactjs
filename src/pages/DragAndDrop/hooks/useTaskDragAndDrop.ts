...
import { DragItem, ICard } from '../utils/interface';
import { ItemType } from '../utils/enum/Status';

export function useTaskDragAndDrop<T extends HTMLElement>(
  { card, index }: { card: ICard; index: number },
  ...
  const [{ isDragging }, drag] = useDrag<
    ...
    { isDragging: boolean }
  >({
    item: { from: card.status, id: card.id, index },
    type: ItemType.Card,
    collect: (monitor) => ({
